...
const els = {};
["q", "count", "pgCount", "facets", "clsFacets", "onlyTx", "typeFacets", "onlyDiagram",
...
function renderCatFacets() {
  ...
  const cls = DATA.facets.classification || {};
  const top = Object.entries(cls).slice(0, 40);
  els.clsFacets.innerHTML = top.length
    ? top.map(([n, c]) => `<button class="chip" data-cls="${esc(n)}">${esc(n)}${ndcHint(n)} <span>${c}</span></button>`).join("")
    : `<span class="muted-note">（分類クロール完了後に表示）</span>`;
  for (const b of els.clsFacets.querySelectorAll(".chip"))
    b.addEventListener("click", () => { toggleSet(state.cls, b.dataset.cls); b.classList.toggle("on"); renderCatalog(); });
}
// NDC第1区分のヒント（先頭1桁）
const NDC1 = { "0": "総記", "1": "哲学", "2": "歴史", "3": "社会", "4": "自然", "5": "技術", "6": "産業", "7": "芸術", "8": "言語", "9": "文学" };
function ndcHint(code) { const d = (code || "").match(/^(\d)/); return d ? `<span class="ndc">${NDC1[d[1]] || ""}</span>` : ""; }
function renderCatalog() {
  const terms = termsOf(els.q.value);
  const matched = [];
  for (const r of DATA.records) {
    if (state.cats.size && !state.cats.has(r.category)) continue;
    if (state.cls.size && !state.cls.has(r.classification)) continue;
    const hasPages = r.pages && r.pages.length;
    if (state.onlyTx && !hasPages) continue;
    const metaText = `${r.title} ${r.yomi} ${r.author} ${r.classification || ""} ${r.call_number || ""}`;
...
function pageRow(p, terms, r, full = false) {
  ...
  const srcb = p.source === "ndlkotenocr-lite"
    ? `<span class="srcbadge" title="NDL古典籍OCR-Lite (CC BY 4.0) による翻刻">NDL翻刻</span>`
    : (p.source === "claude" || !p.source ? `<span class="srcbadge claude" title="Claude vision による構造化翻刻">AI構造化</span>` : "");
  return `<div class="page" data-media="${p.media_pkey}">
    <img loading="lazy" src="${p.thumb}" data-full="${p.image_full}" alt="第${p.order + 1}画像">
    <div class="pbody">
      <div class="plabel">第 ${p.order + 1} 画像 ${t} ${srcb} ${conf}
